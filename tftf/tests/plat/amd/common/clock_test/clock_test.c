...
{
	int32_t test_clock_num = (int32_t)test_clock_list_size;
	uint32_t result = 0U;
	int32_t status, i;
	...
	for (i = 0; i < test_clock_num; i++) {
		...
		if (!pm_clock_control_is_refused(clock_id)) {
			tftf_testcase_printf("%s ERROR clock 0x%x controlled without "
					     "device request\n", __func__, clock_id);
			return TEST_RESULT_FAIL;
		}
	}
...
{
	int32_t test_clock_num = (int32_t)test_clock_list_size;
	int32_t parent_id = PM_CLK_TEST_PARENT;
	int32_t divider = 10U;
	uint32_t result = 0U;
	int32_t status, i;
	...
	for (i = 0; i < test_clock_num; i++) {
		...
		status = xpm_request_node(device_id, PM_CAP_ACCESS, 0,
					  PM_REQ_ACK_DEFAULT);
		if (status != PM_RET_SUCCESS) {
		...
		status = xpm_clock_set_divider(clock_id, PM_CLK_SET_DIVIDER(divider));
		if (status != PM_RET_SUCCESS) {
		...
		if ((status != PM_RET_SUCCESS) ||
		    (divider != (int32_t)(result & PM_CLK_DIVIDER_MASK))) {
			tftf_testcase_printf("%s ERROR to set the divider for 0x%x clock, "
					     "Status: 0x%x\n", __func__, clock_id, status);
			return TEST_RESULT_FAIL;
		}
