...
#define SKIP_TEST_IF_AIE_NOT_SUPPORTED()					\
	do {									\
		if(!is_feat_aie_supported()) {					\
			tftf_testcase_printf("FEAT_AIE not supported\n");	\
			return TEST_RESULT_SKIPPED;				\
		}								\
	} while (false)
...
#define SKIP_TEST_IF_HACDBS_NOT_SUPPORTED()					\
	do {									\
		if (!is_feat_hacdbs_supported()) {				\
			tftf_testcase_printf("HACDBS not supported\n");		\
			return TEST_RESULT_SKIPPED;				\
		}								\
	} while (0)

#define SKIP_TEST_IF_SRMASK_NOT_SUPPORTED()					\
	do {									\
		if (!is_feat_srmask_supported()) {				\
			tftf_testcase_printf("SRMASK not supported\n");		\
			return TEST_RESULT_SKIPPED;				\
		}								\
	} while (0)

#define SKIP_TEST_IF_SPE_NOT_SUPPORTED()					\
	do {									\
		if (!is_feat_spe_supported()) {					\
			tftf_testcase_printf("SPE not supported\n");		\
			return TEST_RESULT_SKIPPED;				\
		}								\
	} while (0)

#define SKIP_TEST_IF_SPE_EXC_NOT_SUPPORTED()					\
	do {									\
		if (!is_feat_spe_exc_supported()) {				\
			tftf_testcase_printf("SPE_EXC not supported\n");	\
			return TEST_RESULT_SKIPPED;				\
		}								\
	} while (0)

#define SKIP_TEST_IF_SPE_NVM_NOT_SUPPORTED()					\
	do {									\
		if (!is_feat_spe_nvm_supported()) {				\
			tftf_testcase_printf("SPE_nVM not supported\n");	\
			return TEST_RESULT_SKIPPED;				\
		}								\
	} while (0)
